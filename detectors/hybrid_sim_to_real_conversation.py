"""
detectors/hybrid_sim_to_real_conversation.py
--------------------------------------------
Builds the few-shot Conversation for detecting
Hybrid (Sim-to-Real) data collection in RL code.

Prompt files (all under prompts/hybrid_sim_to_real/):
...
_PROMPT_DIR = _PROJECT_ROOT / "prompts" / "hybrid_sim_to_real"
...
def _read(filename: str) -> str:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Expected files:\n"
            f"  prompts/hybrid_sim_to_real/system.txt\n"
            f"  prompts/hybrid_sim_to_real/question.txt\n"
            f"  prompts/hybrid_sim_to_real/sample_context.txt\n"
            f"  prompts/hybrid_sim_to_real/sample_answer.txt"
        )
...
def create_python_rl_hybrid_sim_to_real_conversation(
    target_context: str,
) -> Conversation:
    """
    Build a few-shot Conversation for the LLM to detect
    Hybrid (Sim-to-Real) data collection patterns.
