"""
detectors/offline_batch_conversation.py
---------------------------------------
Builds the few-shot Conversation for detecting
Offline (Batch) data collection in RL code.

Prompt files (all under prompts/offline_batch/):
...
_PROMPT_DIR = _PROJECT_ROOT / "prompts" / "offline_batch"
...
def _read(filename: str) -> str:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Expected files:\n"
            f"  prompts/offline_batch/system.txt\n"
            f"  prompts/offline_batch/question.txt\n"
            f"  prompts/offline_batch/sample_context.txt\n"
            f"  prompts/offline_batch/sample_answer.txt"
        )
...
def create_python_rl_offline_batch_conversation(
    target_context: str,
) -> Conversation:
    """
    Build a few-shot Conversation for the LLM to detect
    Offline (Batch) data collection patterns.
